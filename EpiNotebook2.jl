...
# ╔═╡ 7d6bc200-ca16-11ef-3e50-afd31089de05
using OrdinaryDiffEqTsit5, PlotlyLight, PlutoUI, PlutoTeachingTools

# ╔═╡ 568c6719-1937-4875-a353-0af45eed59f1
import PlutoUI: combine

# ╔═╡ a90587e2-f7e9-4c8c-805f-35162036c211
TableOfContents() 

# ╔═╡ b9ea8aa7-3af1-4026-bfa7-b4193ec8ebb8
md"""
...
# ╔═╡ d8e08366-3d20-485d-9fb3-8d3257a0f60b


# ╔═╡ d3ca2d52-a24d-4c20-bb40-bc6c1df5e0a9
md"""
...
# ╔═╡ ed33b5fc-802c-472b-9ba9-d22c18621d26
md"""
First, we will define the SIR model as a function which is designed to be compatible with the OrdinaryDiffEq package - which is used to simulate differential equations. In the below code block:
...
# ╔═╡ d10279e4-1fec-4b42-af2b-3dfde7558d1f
...
# ╔═╡ 61894ca6-bb4a-4610-a125-2217738ea66c
md"""
...
# ╔═╡ c137360f-727d-47c3-9e2e-5c43ddda61bd
begin
...
function epi_input(rates::Vector)
	...
	return combine() do Child
		...
		inputs = [
			md""" $(name): $(
				Child(name, Slider(0.01:0.01:2.0, default = vals[findfirst(rates.==name)], show_value = true))
			)"""
		...
		md"""
		**SIR model epidemiological parameters:**
		$(inputs)
		"""
	end
end

# ╔═╡ 23eed285-2a2f-4697-b7f3-ab50fc40a5d1
@bind speeds epi_input(["β", "γ"])

# ╔═╡ 0ffd7e4d-67a6-406b-a932-946bfe027f9f
begin
		parameters = [speeds.β, speeds.γ]
		SIRproblem = ODEProblem(SIRmodel!, u0, tspan, parameters)
		solveSIR = solve(SIRproblem, Tsit5(), saveat=tres)
		#p = plot(solveSIR,labels=["S" "I" "R"]) 
		p = plot(x = solveSIR.t, y = solveSIR[1,:], type="scatter",name="S").scatter(x = solveSIR.t, y = solveSIR[2,:],name="I").scatter(x = solveSIR.t, y = solveSIR[3,:],name="R")
		p.layout.title.text = "Outbreak simulation for the SIR model"
		p.layout.yaxis.title.text = "Population fraction"
		p.layout.xaxis.title.text = "Time (days)"
		p
end

# ╔═╡ 0fa688a7-ba91-4b0c-8210-a514c5618378
question_box(md"How many different categories of dynamic outcomes are there?")

# ╔═╡ cbea180f-55ef-42ae-ae4b-069827a41a1e
md"""
With some of the above parameter combinations the disease can take off and many (sometimes nearly all) people can become infected. But, this is not always the case. For some combinations of parameters the disease fails to spread much further than the intial infected population fraction.
...
# ╔═╡ 62f1562d-8d0e-4823-bc9a-cc955263a14f
...
# ╔═╡ 5b79d90e-5df0-43bf-9dfe-c1c269cbaae5
md"""
The SIR model is very simple, which is useful for understanding core epidemiology concepts, but also means that it is missing many elements and heterogeneities that are present in real contexts where several of the underlying assumptions may not hold. A central goal for today is to consider what the shortcomings of the SIR model are.

One key feature that the SIR model is unable to capture is the possibility of endemic diseases e.g., consider the flu which comes back year after year. Mathematically, this is because the SIR model only considers the depletion of susceptibles. Biologically, new susceptible individuals could be a result of the recovered individuals losing their acquired immunity (a.k.a waning immunity), or could be from new individuals appearing in the population - either as new births, or by migration.
"""

# ╔═╡ b948cbe8-ee49-407d-9349-7fa674957cbb
md"""
# 2) Plotly interactivity
Did you notice the interactive elements of the outbreak simulation above? If you move your cursor over the figure you will notice some icons appear above the title. These allow you to change the scale and move around in the figure, as well as to download a figure! Also, if you click on items in the legend (S,I,R) on the right of the figure you can make these lines appear, or disappear. This can be very helpful if there are a lot of lines on one figure! Finally, do note the tooltip that appears when you hover over a line -- which shows the x,y co-ordinates.

Go back and try this functionality!
"""

# ╔═╡ 086e1b92-c3cb-49aa-a620-56b2fbada2a9
md"""
# 3) Endemic outbreaks and waning immunity

"""

# ╔═╡ cc048b2e-b082-4414-b9b0-15a65c8f0402
md"""
...
# ╔═╡ 258689c8-3d31-4676-abf4-ae51c7ce0f30
md"""
# 5) The stochastic SIR model
...
manifest_format = "2.0"
project_hash = "9858a395ef2bc5b17698516e250203f1816c6096"
